...
public class MyLevelGenerator{
	...
	private final String LANGUAGE = "abcdefghijklmnopqrstuvwxyz";
	private final int LEVEL_LENGTH = 100;
	private final double THRESHOLD_FITNESS = 0.85; // fitness threshold for terminating the genetic algorithm
	private final int THRESHOLD_COUNT = 200; // iteration threshold for terminating the genetic algorithm
	private final int THRESHOLD_POPULATION = 500;
	private final int NUM_CROSSOVERS_PER_ITERATION = 50;
	private final boolean COMPETE_WITH_PARENTS = false;
	...
	private MyDNA generateRandomIndividual ()
	{
		...
		for(int i = 0; i < LEVEL_LENGTH; i++){
			int newCharIndex = r.nextInt(LANGUAGE.length());
			newChromosome += LANGUAGE.charAt(newCharIndex);
		}
	...
	private boolean terminate (ArrayList<MyDNA> population, int count)
	{
		...
		// YOUR CODE GOES BELOW HERE
		if(count > THRESHOLD_COUNT) return true;

		for(int i = 0; i < population.size(); i++){
			if(population.get(i).getFitness() > THRESHOLD_FITNESS) return true;
		}
		// YOUR CODE GOES ABOVE HERE
		return decision;
	}
	...
	private ArrayList<MyDNA> selectIndividualsForMutation (ArrayList<MyDNA> population)
	{
		...
		// YOUR CODE GOES BELOW HERE
		int numChosen = r.nextInt(population.size());
	...
	private int numberOfCrossovers ()
	{
		...
		// YOUR CODE GOES BELOW HERE
		num = NUM_CROSSOVERS_PER_ITERATION;
		// YOUR CODE GOES ABOVE HERE
		return num;
	...
	private MyDNA pickIndividualForCrossover (ArrayList<MyDNA> population, MyDNA excludeMe)
	{
		MyDNA picked = null;
		// YOUR CODE GOES BELOW HERE
		int chosenIndex = r.nextInt(population.size());
		picked = population.get(chosenIndex);
		// YOUR CODE GOES ABOVE HERE
		if (picked == excludeMe) {
			return null;
		}
	...
	private ArrayList<MyDNA> competeWithParents (ArrayList<MyDNA> oldPopulation, ArrayList<MyDNA> newPopulation, Hashtable parents)
	{
		...
		// not needed since competeWithParentsOnly() returns false
	...
	private ArrayList<MyDNA> globalCompetition (ArrayList<MyDNA> oldPopulation, ArrayList<MyDNA> newPopulation)
	{
		ArrayList<MyDNA> finalPopulation = new ArrayList<MyDNA>();
		// YOUR CODE GOES BELOW HERE
		ArrayList<MyDNA> combinedPopulation = new ArrayList<MyDNA>();
		combinedPopulation.addAll(oldPopulation);
		combinedPopulation.addAll(newPopulation);

		for(int i = 0; i < this.getPopulationSize(); i++){
			MyDNA fittestIndividual = getBestIndividual(combinedPopulation);
			finalPopulation.add(fittestIndividual);
			combinedPopulation.remove(fittestIndividual);
		}
		// YOUR CODE GOES ABOVE HERE
		if (finalPopulation.size() != this.getPopulationSize()) {
			throw new IllegalStateException("Population not the correct size.");
