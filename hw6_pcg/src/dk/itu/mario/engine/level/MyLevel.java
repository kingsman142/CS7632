...
public class MyLevel extends Level{
	...
	public void create(MyDNA dna, int type)
	{
        ...
        // straight path for mario to walk on
        for(int i = STARTOFFSET; i < (width - EXITOFFSET); i++){
            this.setBlock(i, DEFAULTHEIGHT-2, HILL_TOP);
            this.setBlock(i, DEFAULTHEIGHT-1, GROUND);
        }

		// simplest strategy - just read/scan over the chromosome and let each character represent a type of block
        String chromosome = dna.getChromosome();
        for(int i = 0; i < chromosome.length(); i++){
            char blockType = chromosome.charAt(i);
            if(blockType == 'm' || blockType == 'c'){
                this.setBlock(i + STARTOFFSET, DEFAULTHEIGHT - 3, COIN);
            } else if(blockType == 'p'  || blockType == 'j' || blockType == 't' || blockType == 'v'){
                this.setBlock(i + STARTOFFSET, DEFAULTHEIGHT - 3, ROCK);
			} else if(blockType == 'i' || blockType == 'o'){
				this.setBlock(i + STARTOFFSET, DEFAULTHEIGHT - 3, ROCK);
				this.setBlock(i + STARTOFFSET, DEFAULTHEIGHT - 4, ROCK);
            } else if(blockType == 'u' || blockType == 's' || blockType == 'k'){
