...
def myBuildTree(agent):
	...
	root = makeNode(MyRoot, agent, "root")
	root_selector = makeNode(Selector, agent, "root_selector")

	retreat = makeNode(Retreat, agent, HEALTH_THRESHOLD, "retreat")
	dont_heal = makeNode(HitpointDaemon, agent, HEALTH_THRESHOLD, "dont_heal")
	buff = makeNode(BuffDaemon, agent, BUFF_THRESHOLD, "buff")
	dont_heal_selector = makeNode(Selector, agent, "dont_heal_selector")

	destroy_enemy_hero = makeNode(Sequence, agent, "destroy_enemy_hero")
	chase_enemy_hero = makeNode(ChaseHero, agent, "chase_enemy_hero")
	kill_enemy_hero = makeNode(KillHero, agent, "kill_enemy_hero")

	destroy_enemy_minion = makeNode(Sequence, agent, "destroy_enemy_minion")
	chase_enemy_minion = makeNode(ChaseMinion, agent, "chase_enemy_minion")
	kill_enemy_minion = makeNode(KillMinion, agent, "kill_enemy_minion")

	root.addChild(root_selector)
	root_selector.addChild(retreat)
	root_selector.addChild(dont_heal)

	dont_heal.addChild(dont_heal_selector)
	dont_heal_selector.addChild(buff)
	dont_heal_selector.addChild(destroy_enemy_minion)

	buff.addChild(destroy_enemy_hero)
	destroy_enemy_hero.addChild(chase_enemy_hero)
	destroy_enemy_hero.addChild(kill_enemy_hero)

	destroy_enemy_minion.addChild(chase_enemy_minion)
	destroy_enemy_minion.addChild(kill_enemy_minion)
	### YOUR CODE GOES ABOVE HERE ###
	return root
...
class MyRoot(BTNode):
	...
	def execute(self, delta):
		...
		# selector node is its only child
		selector_output = self.children[0].execute()

		# perform a couple necessary actions at each timestep
		find_and_attack_closest_enemy(self.agent)
		deploy_areaeffect(self.agent)
		dodge_enemy_bullet(self.agent)

		return selector_output

def deploy_areaeffect(agent):
	if not agent.canAreaEffect():
		return

	enemies = agent.world.getEnemyNPCs(agent.getTeam())
	return agent.areaEffect() if (not enemies is None and len(enemies) > 0) else False

def find_and_attack_closest_enemy(agent):
	enemies = agent.world.getEnemyNPCs(agent.getTeam())
	if not agent.canFire() or enemies is None or not len(enemies):
		return

	# find closest enemy
	enemy_distances = {}
	for enemy in enemies:
		enemy_distances[enemy] = distance(agent.getLocation(), enemy.getLocation())
	sorted_enemies = sorted(enemy_distances.items(), key = lambda pair : pair[1])
	closest_enemy = sorted_enemies[0][0]
...
def dodge_enemy_bullet(agent):
	# find all bullets in the game, then only keep the ones that belong to enemy agents
	list_of_game_bullets = (agent.getVisibleType(Bullet))
	agent_team = agent.getTeam()
	enemy_bullets = [bullet for bullet in list_of_game_bullets if agent_team == bullet.getOwner().getTeam()] # only keep enemy bullets

	# sort bullets by distance
	enemy_bullet_distances = {bullet : distance(agent.getLocation(), bullet.getLocation()) for bullet in enemy_bullets}
	sorted_enemy_bullets = sorted(enemy_bullet_distances.items(), key = lambda pair : pair[1])

	# find the trajectory of the bullet, and if it's going to eventually hit the player, dodge
	for bullet, dist in sorted_enemy_bullets:
		angle = math.radians(bullet.orientation)
		bullet_trajectory_endpoint = (bullet.getLocation()[0] + 1000*math.cos(angle), bullet.getLocation()[1] + -1000*math.sin(angle)) # extrapolate out the trajectory line

		if minimumDistance([bullet.getLocation(), bullet_trajectory_endpoint], agent.getLocation()) < agent.getMaxRadius(): # the bullet is going to hit the agent, so dodge
			agent.dodge((angle + math.pi / 2))
