...
class Sequence(BTNode):
	...
	def execute(self, delta = 0):
		...
		curr_index = self.getCurrentIndex()
		if self.getNumChildren() == 0 or curr_index >= num_children:
			return True

		curr_child = self.getChild(curr_index)
		child_status = curr_child.execute(delta)
		if child_status in [None, False]: # child is either running or it failed
			return child_status
		else:
			self.setCurrentIndex(curr_index + 1) # check if child succeeded. if so, update child index, go to next child, and repeat
			return None
...
class Selector(BTNode):
	...
	def execute(self, delta = 0):
		...
		curr_child = self.getChild(curr_index)
		child_status = curr_child.execute(delta)
		if child_status in [None, True]: # child is either running or it succeeded
			return child_status
		else:
			self.setCurrentIndex(curr_index + 1) # check if child failed. if so, update child index, go to next child, and repeat
			return None
		### YOUR CODE GOES ABOVE HERE ###
		return False
